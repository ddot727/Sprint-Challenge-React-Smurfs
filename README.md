...
`Client-side Routing` is where the user recieves all of the information needed to run the entire site on the first load up and doesn't need to refresh after every request. `Server-side routing` is where the data is handled by a server, making the browser refresh after every request, but initially loads up much faster than Client-side routing.
